...
if($_SERVER["REQUEST_METHOD"] == "POST"){ 
    ...
    $paymodechange = $obj['paymodechange'];  
    $valq = "";
    ...
    }else if($paymodechange){
        $query = "update tblbilling set date=now(), paymode=$paymode where billno = $billno";        
        if($con->query($query) === TRUE){
            $con->commit();
            echo json_encode(array("status"=>TRUE, "billno"=>$billno, "msg"=>"Bill ( Paymode ) Successfully Updated"));          
        }else{
            $con->rollback();
            echo  json_encode(array("status"=>FALSE,"msg"=>"Bill Not Updated billing".$con->error));
        }
...
if($_SERVER["REQUEST_METHOD"] == "GET"){   
    ...
    if(startsWith($_SERVER['REQUEST_URI'], "/recentbill")){
        ...
        $result = executeSelect("select totalamount,paymode from tblbilling where billno=".numberOnly($billno));
        if($result){ // if the billno not present it resturns FALSE
            $row = $result->fetch_assoc();
            $obj['status'] = TRUE;
            $obj['totalamt'] = $row['totalamount'];
            $obj['paymode'] = $row['paymode'];
        }else{
            $obj['status'] = FALSE;
        }
    }
